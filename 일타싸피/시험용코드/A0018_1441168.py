...
while True:
    ...
    if i == 1:
        ...
        # 고스트볼(gx, gy 타격점을 찾고)
        # 시작지점부터 홀까 거리와(WH직선)
        # 시작부터 고스트볼(WG) + 고스트볼부터 홀가지 (GH)
        # 거리를 기준으로 피타고라스를 응용하여 둔각인지 아닌지 판별하여
        # 각도를 전달하려했지만 시간내에 완성하지 못하였습니다.
        ##########################################################


        for hx, hy in HOLES:
            now_angle = math.atan2(hy - targetBall_y, hx - targetBall_x)
            L = (hy - targetBall_y) ** 2 + (hx - targetBall_x) ** 2
            gx, gy = targetBall_x - 5.73 * math.sin(now_angle), targetBall_y - 5.73 * math.cos(now_angle)
            wh = (whiteBall_x - hx) ** 2 + (whiteBall_y - hy)
            wg = (whiteBall_x - gx) ** 2 + (whiteBall_y - gy)
            gh = (gx - hx) ** 2 + (gy - hy)
            if wh * 0.95 > wg + gh:
                print(gx, gy)
                width = abs(gx - whiteBall_x)
                height = abs(gy - whiteBall_y)
                radian = math.atan2(height, width)
                angle = (math.degrees(radian) + 269) % 360
                targetBall_x = gx
                targetBall_y = gy
                print(radian, angle)
                break
