...
def enemies_threaten(pos):
    for er, ec, et in find_all_enemies():
...
def choose_shoot_T_or_safe_move(fallback_target=None):
    ...
    for i, (dr, dc) in enumerate(DIRS):
        ...
        if enemies_threaten((nr, nc)):
            continue
        # 목표까지 거리 우선 + 적과의 최소거리 보조
        target_score = distance((nr, nc), fallback_target) if fallback_target else 0
        min_enemy = float('inf')
        for er, ec, _ in find_all_enemies():
            d = distance((nr, nc), (er, ec))
            if d < min_enemy:
                min_enemy = d
        candidates.append((target_score, -min_enemy, i))
...
def apply_safety_guard(action, fallback_target=None):
    global safety_stop_used_this_turn
    ...
    if not in_bounds(nr, nc) or is_forbidden_cell(nr, nc):
        ...
        if alt:
            return alt
        return track_stop_and_return_S()

    # 다음 칸이 적 사격 경로라면
    if enemies_threaten((nr, nc)):
        if not safety_stop_used_this_turn:
            return track_stop_and_return_S()
        else:
            alt = choose_shoot_T_or_safe_move(fallback_target)
            if alt:
        ...
        if path:
            ...
            if d is not None:
                ...
                if in_bounds(nr, nc) and map_data[nr][nc] == 'T':
                    return S_CMD[d]  # 사격은 안전가드 비대상(이동 아님)
...
def finalize_stop_count(cmd):
    """
    이번 턴 최종 명령이 'S' 이고, 아직 카운트에 반영하지 않았다면 카운트를 증가.
    (안전가드로 이미 증가시킨 경우는 중복 방지)
    """
    global S_USAGE_COUNT, stop_counted_this_turn
    if cmd == "S" and not stop_counted_this_turn:
        S_USAGE_COUNT += 1
        stop_counted_this_turn = True
...
if __name__ == "__main__":
    ...
    while game_data is not None:
        ...
        # (디버깅) 원하는 경우 S 누적 출력
        # print(f"[DEBUG] Turn {turn_count} Output: {output}, Total S used: {S_USAGE_COUNT}")
