...
def apply_safety_guard(action):
    ...
    이동명령에 안전가드 적용:
      - 다음 칸이 적의 '차폐 없는' 사격경로면:
          · 위협 적 각각과의 경로에 '다른 아군 탱크'가 껴 있으면 → 이동 허용
          · 그렇지 않으면:
              - S 사용 가능(S<2)이고 이번 턴 안전정지 미사용 → 'S' 1회
              - 아니면 1) 인접 T 사격 2) 안전한 G 이동 3) 원래 이동 강행
      - 안전하면 그대로 이동
    """
    d = cmd_to_dir(action)
    ...
    if threats:
        # 모든 위협에 대해 '다른 아군' 차폐가 있으면 안전
        blocked = all(path_has_other_ally_between(e, next_pos) for e in threats)
        if blocked:
            return action

        # 차폐되지 않은 위협 존재 → S 우선(2회 이하), 그 외 회피
        s_try = None
